...
(function () {
  ...
  class ParticleEngine {
    ...
    createParticle(initial = false) {
      ...
      const baseSpeed = (1.4 + Math.random() * 2.8) * (isSuperFast ? (1.75 + Math.random() * 0.5) : 1.0) * this.speedMultiplier;
    ...
    triggerShatterBurst(x, y, count = 8, isExtra = false) {
      const finalCount = isExtra ? Math.round(count * 1.4) : count;
      for (let i = 0; i < finalCount; i++) {
        const pAngle = Math.random() * Math.PI * 2;
        const pSpeed = (1.5 + Math.random() * 3.8) * (isExtra ? 1.3 : 1.0);
        this.shards.push({
          x: x,
          y: y,
          vx: Math.cos(pAngle) * pSpeed,
          vy: Math.sin(pAngle) * pSpeed,
          radius: 1.0 + Math.random() * (isExtra ? 2.2 : 1.5),
          alpha: 1.0,
  ...
  class SentinelScoreboard {
    ...
    checkMilestones() {
      // 1. Auto-Preset & Speed Hyperdrive Progression every 250 Points!
      ...
      if (this.score >= 250 && targetThemeIndex !== this.currentThemeIndex) {
        ...
        // Dynamic speed boost level
        const level = Math.min(3, Math.floor(this.score / 250));
        const speedMult = 1.0 + (level * 0.15);
